...
def _cfg():
    return {
        ...
        "start_hour": int(os.getenv("CALENDAR_BUSINESS_START_HOUR", "14")),
        "end_hour": int(os.getenv("CALENDAR_BUSINESS_END_HOUR", "17")),
        "duration_min": int(os.getenv("CALENDAR_MEETING_DURATION_MIN", "30")),
...
def _build_description(score: int, insights: dict | None) -> str:
    lines = [f"Voice screening complete. Score: {score}/100", ""]
    if insights:
        ...
        if insights.get("notice_period_weeks") is not None:
            lines.append(f"Notice period: {insights['notice_period_weeks']} weeks")
        exp_sal = (insights.get("expected_salary") or {}).get("raw_text")
        if exp_sal:
            lines.append(f"Expected salary: {exp_sal}")
    lines.append("")
