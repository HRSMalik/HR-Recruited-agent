...
## Iconography & the modern-minimalist bar (what separates polished from cheap)

The fastest tell of an amateur UI is the icon layer and decorative noise. Hold this bar — it is not optional:

- **One cohesive SVG icon set — NEVER emoji.** Icons are inline SVG from a *single* library (Lucide / Phosphor / Heroicons style): uniform stroke (1.5–2px), consistent box size (16/20/24), optically centred, `currentColor`-tinted. **Using system emoji glyphs as UI icons (a mic, camera, home, or checkmark pulled from the emoji font) is an automatic fail** — platform-inconsistent, wrong weight, instantly cheap. Never mix icon sources or stroke weights.
- **Neutral-forward palette; the accent is a guest (60-30-10).** Surfaces, text, and hairlines carry the UI. The brand accent lives in ~10% of the pixels — one primary CTA, the active nav item, a single chart series — not flooded across every card border, heading, and pill. A single saturated hue everywhere reads as a template.
- **Ground the accent in brand or domain — never invent one; if unsure, ASK.** The accent must come from either (a) the company's actual brand colour — grep the codebase/config/logo/existing UI for it — or (b) the product domain's conventions when no brand exists (e.g. for HR/recruitment: blue for trust/professionalism, green for growth/people; for fintech: deep blue/green for trust; for health: calm blue/teal). **If neither is determinable with confidence, ask the user for the field/brand direction before choosing** — do not default to an arbitrary hue (that is exactly why so many tools look generic). Record the chosen accent + its one-line rationale in `project-design-config.md`.
- **Whitespace is the design.** Generous, rhythmic spacing does more for "polished" than any ornament. Airy beats dense-and-bordered; separate sections with space, not by boxing everything in a bordered card.
- **No decoration masquerading as design.** Ban: coloured top-border "accent bars" on cards, gradient-blob avatars / hero fills, faux or scribble sparklines, glow/neon, drop-shadows on everything, rainbow status dots. Depth = one subtle shadow tier + hairline borders, applied consistently. If an element encodes no information, cut it.
- **Refined type & honest data-viz.** A real modular scale with deliberate weight contrast (not everything semibold); tabular numerals in metrics; clean, correctly-baselined charts (real sparkline/area/donut) — never a hand-drawn squiggle.
- **North star.** Calibrate to the quiet confidence of Linear / Vercel / Stripe / Notion — restrained, spacious, typographically disciplined, one accent, crisp icons. If the screen would sit comfortably beside those, it passes; if it looks like a Bootstrap admin template, it fails.
