...
- **triaging an EXTERNALLY-filed bug (a human QA tester's ticket): run P35 then P34 first, before anything else.** **P35** — open the attachment; the description is usually empty and the image *is* the report, and it also tells you the environment, the role, and the filing time. **P34** — validate the premise: is this a real regression, an intentionally reverted/deferred feature, or a surface that never worked? Both are cheap and both routinely change the verdict. Only then map the symptom onto a pattern number.
- **A recurring pattern deserves a CLASS sweep, not a spot fix.** When a ticket is the 3rd+ sighting of a pattern (see P7's history), the finding to report is not just this instance — it is the un-swept remainder. Prefer one mechanical assertion over the whole app (e.g. a regex for bare UUIDs across rendered text) to a per-screen eyeball.
...
- **First seen:** REZ-324 (form), REZ-328 (review page). *Regressions:* REZ-410, REZ-446 (word broken mid-word), REZ-417 / REZ-449 (a critical control — action buttons, search × — clipped/unreachable, not just cosmetic), REZ-477 (long characters wrap across multiple lines — re-filed QA FAILED, still not held), REZ-495 (a long **nav/stepper label** — "Witness & Additional Parties" — escapes its fixed-width sidebar PROGRESS card and the chip below it clips at the card's bottom edge). **Lesson from REZ-495: the overflowing text was a hardcoded app label, not user input** — so a probe that only stresses *input* fields misses it entirely. Sweep the app's own longest static labels (nav items, stepper/section names, tab titles, enum option text) against every fixed-width container too, at the narrowest project viewport.
...
- **⚠️ This pattern's real lesson: fixing an INSTANCE is not fixing the CLASS — sweep every render site.** `First seen` was REZ-321 in **2026-06**, and it is still being re-filed on *new surfaces* more than a month later: REZ-484 (assignment dropdown), BL-BUG-44 (audit detail leaked raw user UUIDs), REZ-477 (claim identity, re-filed QA FAILED), and now **REZ-494** (the carrier-portal Claims **filter** dropdown renders `a65013ae-c611-4546-af6c-6fc254032b9e` as the option label where a manager's name belongs). Each was fixed where it was reported; the class survived because nobody swept the rest.
  **So on any change that touches names/identity, do a CLASS sweep, not a spot check:** enumerate *every* surface that renders a person or record — table cells, **filter and assignment dropdowns** (both the closed control and the open option list), toasts, breadcrumbs, timelines, audit/activity rows, notifications, CSV/PDF exports — and assert **no bare UUID/ObjectId appears in any visible text**. This is cheap and mechanical: a single regex over the rendered DOM text (`[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}` and 24-hex-char Mongo ids) catches the whole family in one pass — run it on every screen, not just the one in the ticket. Note it fires on the **option list**, which stays invisible until a dropdown is opened, so the sweep must open every select.
...
> **Batch 5 (appended 2026-07-24).** P33 is the INVERSE of P13 — a self-audit pattern distilled from the fleet's own false-positive filings (BL-BUG-40, BL-BUG-37 on the Rezolve project), not from the human tester. It exists to stop the fleet filing defects that aren't real. See `shared/guardrails.md` §9 (no unreproduced findings).

### P33 · False defect — the finding isn't real (self-audit, the inverse of P13) ⚡
- **Probe (on yourself, before filing):** would this finding survive a check that the thing it names actually EXISTS and was exercised with the REAL shape? Confirm the endpoint/method exists (live OpenAPI / a real call — a 404/405 means no such route), the field/key names match the real store/schema (not guessed camelCase↔snake_case), and the failure **reproduces deterministically** on a clean run.
- **Expected:** every filed defect is independently reproduced against the real target with the real contract; anything not reproduced or aimed at a nonexistent route/field is discarded or downgraded to `observation`.
- **Defect signal (in your OWN output):** a "defect" whose repro hits a route that returns 404/405 (doesn't exist), sends keys the app never reads, misreads a response, or can't be reproduced on a second clean run. That's a **false defect** — the tester's artifact, not a SUT bug.
- **Applies to:** every finding before it's filed — highest-risk when probing by assumption (guessed endpoint/field names) instead of the confirmed live contract. **P13 = the SUT lies that a mutation succeeded; P33 = the TESTER files a bug that was never real.** Run this self-check on every finding, same reflex as running P13 on every mutation.
- **First seen:** BL-BUG-40 (filed `GET /users/{id}` — a route that 405s, doesn't exist) + BL-BUG-37 (filed a rehydration bug using `accidentDate`/`accidentTime`, keys not in the real store `incident.date`/`time`); both closed NOT-A-DEFECT on the build session's audit.

### P34 · "Broken" vs never-built / reverted / never-worked — validate the premise against history ⚡
- **Probe (before treating any "X is not working" as a regression):** check what the code and the project history actually say about X. Three distinct realities hide behind the same symptom: (a) X **worked and broke** → a true regression; (b) X was **deliberately reverted or deferred** → absence is intended, not a defect; (c) X was **marked delivered but never once worked** → new, never-tested surface, not a re-test. Resolve which one *before* filing or fixing.
- **Expected:** every "not working" finding names which of the three it is, with the evidence (the commit/ticket that reverted it, the backlog entry deferring it, or a confirmed-failing call proving it never worked).
- **Defect signal:** a High filed against a feature that was **intentionally removed** — or a ticket closed as delivered whose endpoint has never returned 2xx in any environment.
- **Applies to:** any report phrased as "X not working / missing / gone", and any surface whose ticket is already closed.
- **First seen:** **REZ-492** ("Keep me Sign In not working", High) — the remember-me feature was **deliberately reverted on FE+BE across all branches** after it caused a **production login outage** (the FE shipped `remember_me` before the BE schema deployed, so the live BE 422'd every login); it is deferred pending a BE-first redo, so "not working" is the intended state, not a regression. *Inverse direction:* **BL-BUG-50** — `GET /claims/aging` was marked delivered (`BL-GAP-39`) and **REZ-143** was closed citing it, yet it returned **500 in every environment for its entire existence** (a duplicate `def` had overwritten `_claim_scope_query`, so the name never existed → `NameError`), and it **failed quietly** because the FE caught the error into a banner that reads like "no data" rather than "broken". A closed ticket is not evidence the surface ever worked.
- **Corollary for the build/QA handoff:** reverted and deferred features must be recorded somewhere QA reads, or they get re-filed as Highs every cycle; and a "delivered" claim with **zero prior QA coverage** should be treated as new surface and given real cases.

### P35 · The ticket's DESCRIPTION IS EMPTY — the attachment is the whole bug report ⚡
- **Probe:** never triage a QA-filed ticket from its title alone. Fetch the issue **with its `attachment` field**, download every attachment, and **actually view it**. On this board the description is routinely empty and the screenshot/recording carries 100% of the signal — the red annotation box marks the defect, the URL bar names the environment, the sidebar names the acting role, and the clock gives the filing time.
- **Expected:** the finding you record cites what the image actually shows — screen, role, environment, and the annotated element — not an inference from the title. A title like "UI issue" carries no information at all.
- **Defect signal (in your OWN triage):** a ticket summarized, prioritized, or "fixed" without the attachment ever being opened. Guessing from `Id displaying under managers and adjuster dropdown` would have missed that it is the **filter** dropdown on the **carrier portal**, in the **option list**, on the **qa** host.
- **Applies to:** every externally-filed bug. **Mechanics:** `GET /rest/api/3/issue/{key}?fields=...,attachment` for metadata, then `GET /rest/api/3/attachment/content/{id}` with the same Basic auth for the bytes. PNG/JPG can be read directly; a **`.webm` screen recording cannot be viewed as an image** — say so explicitly and triage from the title + code rather than pretending to have watched it.
- **Also read the environment and the clock off the screenshot — they change the verdict.** A bug filed against **qa** may already be fixed by a batch still sitting on **dev**: compare the filing timestamp against that environment's build marker before calling it open. REZ-494/495 were filed on the qa build *hours before* a validated dev batch was promoted there, and REZ-495's screenshot incidentally shows a symptom (a fully-populated Claimant section reading INCOMPLETE) that the promoted batch fixes — so part of what it depicts was already resolved elsewhere. Equally, note **whose data** the screenshot shows: QA tests on protected fixtures (e.g. the `Cyprus Truck` carrier) that our own runs must never mutate.
- **First seen:** REZ-494 + REZ-495 + REZ-492 (all three filed 2026-07-27 with **empty descriptions**; one is a `.webm`).

### P36 · Layered-guard shadowing — the new specific check sits behind a broader one that always fires first ⚡
- **Probe (run it whenever a release adds a *specific* refusal to an existing path):** before writing the case, find every **pre-existing** check on the same path and work out the order they execute in. Then ask the only question that matters — **is there a reachable state that satisfies every earlier guard and still trips the new one?** Construct that fixture deliberately. If it cannot be constructed through any real journey, the check is unreachable and the case cannot be run.
- **Expected:** the new refusal is triggered **live**, by its own condition, and the run shows *its* message — not a broader guard's. Where that is impossible, the case is reported **unverifiable-by-journey** with the shadowing guard named, never quietly passed on a source read.
- **Defect signal:** the refusal arrives with the **wrong reason**. Your assertion may still be satisfied (something was refused, with the status you expected) while the code you came to test never executed. Per §10c this is a PASS on the rule plus a **separate finding about reachability** — the protection holds, but the specific branch is dead behind a broader one and will read as tested coverage forever.
- **Why it recurs:** a guard is added where its own logic belongs rather than where its condition is first distinguishable, and the broader check upstream is already true for every state that reaches it. Nothing fails, no test goes red, and the new code is never executed once — so the habit is invisible from the inside and repeats release after release.
- **Escalate the class, not the instance:** on the **third** occurrence, stop filing individual items and raise **one design question** about guard ordering on that path. Ten separate correct-but-unreachable findings ask the same question ten times and get answered zero times.
- **Applies to:** any newly added validation, authorization or state-machine refusal on a path that already refuses for other reasons — highest-risk on irreversible or money-moving actions, where the specific guard is usually the one that matters most.
- **First seen:** five consecutive instances across four release batches, each a correct new server-side guard placed beneath a broader status or permission check that reached every real fixture first. In every case the rule held, the product was right, and the new branch had never executed.
